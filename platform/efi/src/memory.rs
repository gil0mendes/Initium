...
//! 
//! On EFI, we don't use the generic memory manage code. This is because,
//! while we're still in boot service mode, the firmware is in control of
//! the memory map and we should use memory allocation services to allocate
//! memory. Since it is possible for the memory map to change underneath us,
//! we cannot just get the memory map once during init and use it with the 
//! generic MM code.
