...
static void serial_console_putc(console_out_t *console, char ch) {
  ...
  // if the ch is equals to '\n' we return ('\r') to the next line
  if (ch == '\n') { serial_console_putc(console, '\r'); }

  // write the character
  port->ops->write(port, ch);

  // wait until transmitter fifo and shifter for port is empty
  while (!port->ops->tx_empty(port)) { arch_pause(); }
}
...
static void serial_console_set_colour(console_out_t *console, colour_t fg, colour_t bg) {
  ...
  if (fg != COLOUR_DEFAULT) {
    console_printf(&port->console, "\033[%s%um",
...
static uint16_t serial_console_getc(console_in_t *console) {
  ...
  if (!port->next_ch) { while (!serial_console_poll(console)); }
...
static bool config_cmd_serial(value_list_t *args) {
  ...
  if (args->count >= 2) {
    if (args->values[1].type != VALUE_TYPE_INTEGER) { goto err_inval; }
  ...
  if (args->count >= 3) {
    if (args->values[2].type != VALUE_TYPE_INTEGER) { goto err_inval; }
  ...
  if (args->count >= 5) {
    if (args->values[4].type != VALUE_TYPE_INTEGER) { goto err_inval; }
