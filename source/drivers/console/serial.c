...
static struct {
  ...
  [COLOR_CYAN]            = { 6, false },
  [COLOR_RED]             = { 1, false },
  [COLOR_MAGENTA]         = { 5, false },
  [COLOR_BROWN]           = { 3, false },
  [COLOR_LIGHT_GREY]      = { 7, false },
  [COLOR_GREY]            = { 0, true },
  [COLOR_LIGHT_BLUE]      = { 4, true },
  [COLOR_LIGHT_GREEN]     = { 2, true },
  [COLOR_LIGHT_CYAN]      = { 6, true },
  [COLOR_LIGHT_RED]       = { 1, true },
  [COLOR_LIGHT_MAGENTA]   = { 5, true },
  [COLOR_YELLOW]          = { 3, true },
  [COLOR_WHITE]           = { 7, true },
};
...
static void serial_console_set_color(console_out_t *console, color_t fg, color_t bg) {
  serial_port_t *port = container_of(console, serial_port_t, out);
  ...
  if (fg != COLOR_DEFAULT) {
    serial_port_printf(
      port, "\033[%s%um",
      (color_table[fg].bold) ? "1;" : "", 30 + color_table[fg].num);
  }

  if (bg != COLOR_DEFAULT) {
    serial_port_printf(
      port, "\033[%s%um",
      (color_table[bg].bold) ? "1;" : "", 40 + color_table[bg].num);
  }
}
...
static console_out_ops_t serial_console_out_ops = {
  ...
  .set_color = serial_console_set_color,
